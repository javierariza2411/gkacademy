...
type Service = { _id: string; name: string; description: string; price: number; durationMinutes: number };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (storedUser.role === 'STUDENT' || storedUser.role === 'CUSTOMER') api('/rentals/me').then(setRentals).catch(() => {});
  ...
  if (user.role === 'ADMIN' || user.role === 'COACH') return <main className="dashboard"><div className="container">
    ...
    {user.role === 'ADMIN' && <section className="adminQuickNav" aria-label="Acciones rápidas"><div className="adminQuickIntro"><div className="kicker">Gestión rápida</div><h3>Administra tu academia</h3><p className="muted">Accede a las áreas principales del panel.</p></div><Link className="adminQuickCard" href="/planes"><span className="quickIcon">01</span><span><strong>Planes</strong><small>Precios, sesiones y modalidades</small></span><span className="quickArrow">↗</span></Link><Link className="adminQuickCard" href="/catalogo"><span className="quickIcon">02</span><span><strong>Servicios</strong><small>Catálogo y tarifas disponibles</small></span><span className="quickArrow">↗</span></Link><Link className="adminQuickCard" href="/usuarios"><span className="quickIcon">03</span><span><strong>Personas</strong><small>Profesores y estudiantes</small></span><span className="quickArrow">↗</span></Link></section>}
  ...
  return <main className="dashboard"><div className="container"><div className="studentWelcome"><div><div className="kicker">Mi cuenta</div><h2>Hola, {user.name}</h2><p className="muted">Consulta tus planes, servicios y solicitudes desde un solo lugar.</p></div><Link className="btn" href="/alquiler-arquero">Solicitar arquero <span>↗</span></Link></div><section className="card studentPanel"><div className="sectionTitle"><div><div className="kicker">Tu entrenamiento</div><h3>Mis planes</h3></div><Link className="textLink" href="/#planes">Explorar planes ↗</Link></div>{subscriptions.length ? <div className="studentPlans">{subscriptions.map(subscription => <article className="studentPlan" key={subscription._id}><div><span className="statusPill statusConfirmed">{subscription.status === 'ACTIVE' ? 'Activo' : subscription.status}</span><h3>{subscription.plan?.name || subscription.planName || 'Plan de entrenamiento'}</h3><p className="muted">Tu plan está registrado en la academia.</p></div><Link className="btn secondary darkButton" href="/#planes">Ver planes</Link></article>)}</div> : <div className="emptyPanel"><strong>Aún no tienes un plan activo</strong><span className="muted">Elige una modalidad de entrenamiento para comenzar.</span><Link className="btn" href="/#planes">Ver planes disponibles</Link></div>}</section><section className="card studentPanel"><div className="sectionTitle"><div><div className="kicker">Servicio por partido</div><h3>Mis solicitudes de arquero</h3></div><Link className="textLink" href="/alquiler-arquero">Nueva solicitud ↗</Link></div>{rentals.length ? <div className="studentRequests">{rentals.map(rental => <article className="studentRequest" key={rental._id}><div className="requestDate"><strong>{rental.date}</strong><span>{rental.time}</span></div><div className="requestInfo"><strong>{rental.location}</strong><span>{rental.matchType || 'Partido'} · {rental.durationMinutes || 90} min</span>{rental.assignedGoalkeeper && <span className="assignedInfo">Arquero: {rental.assignedGoalkeeper}</span>}</div><span className={`statusPill ${statusClass[rental.status] || ''}`}>{statusLabels[rental.status] || rental.status}</span></article>)}</div> : <div className="emptyPanel"><strong>No tienes solicitudes de arquero</strong><span className="muted">Solicita uno para tu próximo partido y consulta aquí su estado.</span><Link className="btn" href="/alquiler-arquero">Solicitar un arquero</Link></div>}</section><section className="studentServices"><div className="sectionTitle"><div><div className="kicker">También puedes elegir</div><h3>Servicios disponibles</h3></div><Link className="textLink" href="/catalogo">Ver catálogo ↗</Link></div><div className="serviceChoices">{services.slice(0, 3).map(service => <article className="serviceChoice" key={service._id}><span className="serviceNumber">+</span><h3>{service.name}</h3><p className="muted">{service.description}</p><strong>{money(service.price)} <small>/ servicio</small></strong><Link href={service.name.toLowerCase().includes('arquero') ? '/alquiler-arquero' : '/catalogo'}>Ver servicio ↗</Link></article>)}</div></section></div></main>;
}
